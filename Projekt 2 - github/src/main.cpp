...
void runTests(std::unique_ptr<Graph>& graph, const std::string& representation, int numVertices, double density) {
    ...
    // Print results
    std::cout << "Average time for Kruskal's Algorithm: " << avgKruskalTime << " ms (Expected: " << kruskalComplexity << " ms)" << std::endl;
    std::cout << "Average time for Prim's Algorithm: " << avgPrimTime << " ms (Expected: " << primComplexity << " ms)" << std::endl;
    std::cout << std::endl;
}
